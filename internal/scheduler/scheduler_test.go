...
func TestReconcileSkipsInactivePermits(t *testing.T) {
	...
	if err := st.SetRule(ctx, owner, act, 0, today, veh); err != nil {
		t.Fatal(err)
	}
	...
	if err := st.SetRule(ctx, owner, exp, 0, today, veh); err != nil {
		t.Fatal(err)
	}
...
func TestDriftBackoffSurvivesAFailedCheckpoint(t *testing.T) {
	...
	if err := st.SetRule(ctx, owner, pid, 0, time.Monday, vid); err != nil {
...
func TestWarnExternallyDisplacedRosterGate(t *testing.T) {
	...
	if err := st.SetRule(ctx, owner, pid, 0, time.Now().In(s.loc).Weekday(), vid); err != nil {
		t.Fatal(err)
	}
	s.warnExternallyDisplaced(ctx, p, "MUM123")
...
func TestTenantBusyTellsTheUserEventually(t *testing.T) {
	...
	if err := st.SetRule(ctx, owner, pid, 0, time.Now().In(time.UTC).Weekday(), veh); err != nil {
		t.Fatal(err)
	}
