...
func TestOnboardNudgeMessage(t *testing.T) {
	subject, body := onboardNudgeMessage("resident@example.com", "https://p.stonn.example", (&Service{}).councilOf(context.Background(), ""))
	...
	_, body = onboardNudgeMessage("resident@example.com", "", (&Service{}).councilOf(context.Background(), ""))
