...
func TestOnboardNudgeMessage(t *testing.T) {
	subject, body := onboardNudgeMessage("resident@example.com", "https://p.stonn.example")

	if subject == "" || strings.Contains(subject, "\n") {
		t.Fatalf("subject unusable: %q", subject)
	}
	for _, want := range []string{
		CouncilPasswordResetURL,           // remedy 1: reset, deep-linked
		...
		CouncilRegisterURL,                // the no-account-at-all reader gets the sign-up door
