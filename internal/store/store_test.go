...
func TestCopySchedule(t *testing.T) {
	...
	if err := s.SetRule(ctx, owner, src, 0, time.Monday, veh); err != nil {
		t.Fatal(err)
	}
	if err := s.SetRule(ctx, owner, src, 0, time.Friday, veh); err != nil {
		t.Fatal(err)
	}
	...
	if err := s.SetRule(ctx, owner, dst, 0, time.Wednesday, otherVeh); err != nil {
		t.Fatal(err)
	}
...
func TestPermitMetaAndExpiryReminder(t *testing.T) {
	...
	if err := s.UpdatePermitMeta(ctx, owner, s.DefaultTenant, "555", "Granted", "VPP24714", "(A) 1st Visitor Permit", end); err != nil {
		t.Fatal(err)
	}
	...
	if err := s.UpdatePermitMeta(ctx, owner, s.DefaultTenant, "555", "Granted", "VPP24714", "(A) 1st Visitor Permit", end2); err != nil {
		t.Fatal(err)
	}
	if p, _ := s.GetPermit(ctx, pid); p.ExpiryReminded {
	...
	if err := s.UpdatePermitMeta(ctx, "other@example.com", s.DefaultTenant, "555", "Cancelled", "X", "X", time.Time{}); err != nil {
		t.Fatal(err)
	}
...
func TestDeleteAllForOwner(t *testing.T) {
	...
	for _, owner := range []string{alice, bob} {
		...
		if err := s.SetRule(ctx, owner, pid, 0, time.Monday, veh); err != nil {
			t.Fatal(err)
		}
...
func TestDeletePermit(t *testing.T) {
	...
	if err := s.SetRule(ctx, alice, pid, 0, time.Monday, veh); err != nil {
		t.Fatal(err)
	}
	end := time.Now().Add(time.Hour)
