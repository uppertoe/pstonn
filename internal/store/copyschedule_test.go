...
func copyFixture(t *testing.T, st *Store, owner string) (src, dst, vehID int64) {
	...
	if err := st.SetRule(ctx, owner, src, 0, time.Monday, vehID); err != nil {
		t.Fatalf("rule: %v", err)
	}
	if err := st.SetRule(ctx, owner, src, 0, time.Friday, vehID); err != nil {
		t.Fatalf("rule: %v", err)
	}
...
func TestCopyScheduleReplacesTheTarget(t *testing.T) {
	...
	if err := st.SetRule(ctx, owner, dst, 0, time.Wednesday, vehID); err != nil {
...
func TestCopyScheduleEmptySourceIsNoOp(t *testing.T) {
	...
	if err := st.SetRule(ctx, owner, dst, 0, time.Monday, vehID); err != nil {
		t.Fatal(err)
	}
